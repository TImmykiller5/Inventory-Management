...
const CellAction: React.FC<CellActionProps> = (
    ...
    const onDelete = async (id: string) => {
        try {
            setLoading(true)
            await axios.delete(`/api/purchase/${pathName?.split("/")[1]}/${id}`)
            router.refresh()
            toast({
                title: 'Purchase transaction deleted',
                description: 'The purchase transaction has been deleted successfully',
                duration: 2000,
            })
        } catch (error) {
            ...
            toast({
                title: 'Delete failed',
                description: 'An error occurred while deleting the purchase transaction',
                duration: 2000,
            })
