...
function SideMenu() {
  ...
  const [sessionStorageData, setSessionStorageData] = React.useState<any>();
    ...
    <div className="h-full flex-col justify-between  bg-white hidden lg:flex ">
      <div className="px-4 py-6">
        <nav aria-label="Main Nav" className="mt-6 flex flex-col space-y-1">
          ...
          <Link
            ...
            className="flex items-center gap-2 rounded-lg px-4 py-2 text-gray-500 hover:bg-gray-100 hover:text-gray-700"
          >
            <img alt="sale-icon" src={"/images/supplier-icon.png"} />
      ...
      <div className="sticky inset-x-0 bottom-0 border-t border-gray-100">
        <div className="flex items-center gap-2 bg-white p-4 hover:bg-gray-50">
          <img
            alt="Profile"
            src={sessionStorageData?.imageUrl }
          ...
          <div>
            <p className="text-xs">
              <strong className="block font-medium">
                {sessionStorageData?.firstName + " " + sessionStorageData?.lastName}
              </strong>

              <span> {sessionStorageData?.email} </span>
            </p>
          </div>
        </div>
      </div>
